...
def update_off_boarding(employee_id: str, off_boarding_update: OffBoardingCreate, db: Session = Depends(get_db)):
    ...
    # If status is changed to INITIATED, delete employee from all tables
    if off_boarding_update.status == "INITIATED":
        try:
            ...
            db.execute(
                text("UPDATE assets SET assigned_employee_id = NULL, status = 'Available' WHERE assigned_employee_id = :emp_id"),
                {"emp_id": employee_id}
            )
