...
class ManagerService:
    ...
    def get_leave_balance(db: Session, manager_id: str) -> ManagerBalanceResponse:
        ...
        try:
            result = db.execute(text("SELECT COUNT(*) FROM leave_management WHERE employee_id = :emp_id AND UPPER(status) = 'APPROVED'"), {"emp_id": manager_id}).fetchone()
            total_used = result[0] if result else 0
        except Exception as e:
            db.rollback()
            total_used = 0
        
        return ManagerBalanceResponse(manager_id=manager_id, casual_leave=each_leave, sick_leave=each_leave, earned_leaves=each_leave, total_leaves=total_leaves, employee_used_leaves=total_used, used_casual=0, used_sick=total_used, used_earned=0, remaining_casual=each_leave, remaining_sick=max(0, each_leave - total_used), remaining_earned=each_leave)
...
class HRExecutiveService:
    ...
    def get_leave_balance(db: Session, hr_executive_id: str) -> HRExecutiveBalanceResponse:
        ...
        try:
            result = db.execute(text("SELECT COUNT(*) FROM leave_management WHERE employee_id = :emp_id AND UPPER(status) = 'APPROVED'"), {"emp_id": hr_executive_id}).fetchone()
            total_used = result[0] if result else 0
        except Exception as e:
            db.rollback()
            total_used = 0
        
        return HRExecutiveBalanceResponse(hr_executive_id=hr_executive_id, casual_leave=each_leave, sick_leave=each_leave, earned_leaves=each_leave, total_leaves=total_leaves, employee_used_leaves=total_used, used_casual=0, used_sick=total_used, used_earned=0, remaining_casual=each_leave, remaining_sick=max(0, each_leave - total_used), remaining_earned=each_leave)
