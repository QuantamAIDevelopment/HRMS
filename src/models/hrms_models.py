from sqlalchemy import Column, Integer, String, Date, Time, Text, Numeric, Boolean, DateTime, ForeignKey, CheckConstraint, Index, Float, LargeBinary
...
class Employee(Base):
    ...
    annual_ctc = Column(String(50), server_default='0')
    status = Column(String(50), server_default='inprocess')
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())
    __table_args__ = (
...
class EmployeePersonalDetail(BaseModel):
