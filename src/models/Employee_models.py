...
class Employee(Base):
    ...
    # Relationships
    personal_details = relationship("EmployeePersonalDetails", back_populates="employee",
                                    cascade="all, delete", uselist=False)
    bank_details = relationship("BankDetails", back_populates="employee",
                                cascade="all, delete")
    assets = relationship("Assets", back_populates="assigned_employee")
...
class EmployeePersonalDetails(Base):
    ...
    employee_id = Column(String(50), ForeignKey("employees.employee_id", ondelete="CASCADE"), unique=True)
...
class Assets(Base):
    ...
    __table_args__ = (
        CheckConstraint("status IN ('Assigned','Available','Maintenance')"),
        CheckConstraint("condition IN ('Excellent','Good','Fair')"),
    )
